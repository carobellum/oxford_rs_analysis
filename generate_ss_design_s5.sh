...
# definitions
workDir=/vols/Data/ping/caro/cerebmrsi/bin/ss_designs
...
for id in ${subjList} ; do
    ...
    for cond in ${condList} ; do
      ...
      cp $workDir/ss_template_s5.fsf $workDir/${id}_ses-${cond:0:1}_smooth-5.fsf
      sed -i "s/XXX/"${id}"/g" $workDir/${id}_ses-${cond:0:1}_smooth-5.fsf
      sed -i "s/YYY/ses-"${cond}"/g" $workDir/${id}_ses-${cond:0:1}_smooth-5.fsf
      sed -i "s/ZZZ/ses-"${cond:0:1}"/g" $workDir/${id}_ses-${cond:0:1}_smooth-5.fsf

      # run the .fsf design in feat
     # feat $workDir/${id}_ses-${cond:0:1}_smooth-5.fsf
