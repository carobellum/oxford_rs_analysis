...
for ID in ${subjList}   ; do
    for COND in ${condList} ; do
        for ACQ in ${acqList}   ; do
            ...
            # if [[ -f ${clean}/filtered_func_data_clean.nii.gz ]] && [[ ! -f ${clean}/filtered_func_data_clean_smooth5.nii.gz ]]; then # This line checks if cleaned dataset exists and data hasn't already been registered

                echo Smoothing ${ID} ${COND} ${ACQ} ...

                smooth_job_id=`fsl_sub fslmaths ${clean}/filtered_func_data_clean -s 2.12314225053 ${clean}/filtered_func_data_clean_smooth5`
